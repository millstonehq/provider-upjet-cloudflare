...
func TestProviderConfigExamples(t *testing.T) {
	...
	t.Run("ProviderConfig", func(t *testing.T) {
		...
		if obj.APIVersion != "cloudflare.millstone.tech/v1beta1" {
			t.Errorf("Expected apiVersion 'cloudflare.millstone.tech/v1beta1', got '%s'", obj.APIVersion)
		}
...
func TestResourceExamples(t *testing.T) {
	...
	resourceExamples := map[string]struct {
		...
		"examples/dns/record.yaml": {
			apiGroup: "dns.cloudflare.millstone.tech/v1alpha1",
			kind:     "Record",
		},
	}
...
func TestExampleCombinations(t *testing.T) {
	...
	t.Run("DNS Record with ProviderConfig", func(t *testing.T) {
		files := []string{
		...
		t.Log("DNS Record + ProviderConfig combination validated")
	})
}

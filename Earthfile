...
# Crossplane Provider Cloudflare Build Pipeline
...
schema:
    ...
    RUN PROVIDER_VERSION=$(grep 'TerraformProviderVersion = ' /tmp/cloudflare.go | sed 's/.*"\(.*\)".*/\1/') && \
        ...
        echo "{\"terraform\":[{\"required_providers\":[{\"cloudflare\":{\"source\":\"cloudflare/cloudflare\",\"version\":\"$PROVIDER_VERSION\"}}]}]}" > main.tf.json && \
        tofu init && \
...
generate-raw:
    ...
    COPY +schema/schema.json /app/providers/provider-upjet-cloudflare/config/schema.json
    WORKDIR /app/providers/provider-upjet-cloudflare
...
test:
    ...
    COPY go.mod go.sum examples_test.go /app/providers/provider-upjet-cloudflare/
    WORKDIR /app/providers/provider-upjet-cloudflare
...
test-examples:
    ...
    COPY --dir examples /app/providers/provider-upjet-cloudflare/
    COPY examples_test.go /app/providers/provider-upjet-cloudflare/
    WORKDIR /app/providers/provider-upjet-cloudflare
